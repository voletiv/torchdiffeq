...
parser.add_argument('--n_frames_cond', type=int, default=5)
parser.add_argument('--n_frames_pred', type=int, default=10)
parser.add_argument('--n_frames_future', type=int, default=10)
...
def ode_collate_fn(batch, t_cond):
    ...
    frames_cond, frames_input = frames_input[:, :t_cond], frames_input[:, t_cond:]
...
if __name__ == '__main__':
    ...
    ts_pos = np.linspace(start, stop, num=args.n_frames_pred+args.n_frames_future)
        ...
        for epoch in range(1, args.n_epochs + 1):
            ...
            for b in range(n_batches):
                ...
                for t in range(args.n_frames_cond):
                    obs = vids_cond[vid_ids[b*args.batch_size:(b+1)*args.batch_size], t, :]
                ...
                logpx = log_normal_pdf(vids_pred[vid_ids[b*args.batch_size:(b+1)*args.batch_size]], pred_x, noise_logvar).sum(-1).sum(-1).sum(-1).sum(-1).mean()
            ...
            if epoch % args.vis_step == 0:
                ...
                for t in range(xs_dec_pred_z.shape[1]):
                    xs_t = vids_inf_vis_gpu[:, t]                        # Bx1x64x64
