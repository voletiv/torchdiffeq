...
def get_data_loader(dset_path, num_objects=[1], batch_size=100,
                    n_frames_input=5, n_frames_output=5, n_workers=8, is_train=True):
    transform = transforms.Compose([ToTensor()])
    dset = MovingMNIST(dset_path, is_train, n_frames_input, n_frames_output, num_objects, transform)
    dloader = data.DataLoader(dset, batch_size=batch_size, shuffle=is_train, num_workers=n_workers, pin_memory=True)
    return dloader
